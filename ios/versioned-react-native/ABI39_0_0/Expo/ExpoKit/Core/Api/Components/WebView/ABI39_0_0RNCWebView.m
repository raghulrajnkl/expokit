...
#else
#import <ABI39_0_0React/ABI39_0_0RCTUIKit.h>
#endif // !TARGET_OS_OSX

#import "objc/runtime.h"

static NSTimer *keyboardTimer;
static NSString *const HistoryShimName = @"ABI39_0_0ReactNativeHistoryShim";
...
@interface ABI39_0_0_SwizzleHelperWK : UIView
@property (nonatomic, copy) WKWebView *webView;
@end
@implementation ABI39_0_0_SwizzleHelperWK
...
#if TARGET_OS_OSX
@interface ABI39_0_0RNCWKWebView : WKWebView
@end
@implementation ABI39_0_0RNCWKWebView
- (void)scrollWheel:(NSEvent *)theEvent {
  ABI39_0_0RNCWebView *rncWebView = (ABI39_0_0RNCWebView *)[self superview];
  ABI39_0_0RCTAssert([rncWebView isKindOfClass:[rncWebView class]], @"superview must be an ABI39_0_0RNCWebView");
...
@interface ABI39_0_0RNCWebView () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler,
#if !TARGET_OS_OSX
    UIScrollViewDelegate,
#endif // !TARGET_OS_OSX
    ABI39_0_0RCTAutoInsetsProtocol>

@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onFileDownload;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onLoadingStart;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onLoadingFinish;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onLoadingError;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onLoadingProgress;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onShouldStartLoadWithRequest;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onHttpError;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onMessage;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onScroll;
@property (nonatomic, copy) ABI39_0_0RCTDirectEventBlock onContentProcessDidTerminate;
#if !TARGET_OS_OSX
@property (nonatomic, copy) WKWebView *webView;
#else
@property (nonatomic, copy) ABI39_0_0RNCWKWebView *webView;
#endif // !TARGET_OS_OSX
...
@implementation ABI39_0_0RNCWebView
{
#if !TARGET_OS_OSX
  UIColor * _savedBackgroundColor;
#else
  ABI39_0_0RCTUIColor * _savedBackgroundColor;
#endif // !TARGET_OS_OSX
  BOOL _savedHideKeyboardAccessoryView;
  BOOL _savedKeyboardDisplayRequiresUserAction;

  // Workaround for StatusBar appearance bug for iOS 12
  // https://github.com/ABI39_0_0React-native-community/ABI39_0_0React-native-webview/issues/62
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    #else
    super.backgroundColor = [ABI39_0_0RCTUIColor clearColor];
    #endif // !TARGET_OS_OSX
    _bounces = YES;
    _scrollEnabled = YES;
    ...
    #if !TARGET_OS_OSX
    _savedStatusBarStyle = ABI39_0_0RCTSharedApplication().statusBarStyle;
    _savedStatusBarHidden = ABI39_0_0RCTSharedApplication().statusBarHidden;
    #endif // !TARGET_OS_OSX
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[ABI39_0_0RNCWKProcessPoolManager sharedManager] sharedProcessPoolForScopeKey:self.scopeKey];
  }
  ...
  [wkWebViewConfig.userContentController addScriptMessageHandler:[[ABI39_0_0RNCWeakScriptMessageDelegate alloc] initWithDelegate:self]
...
#else
    _webView = [[ABI39_0_0RNCWKWebView alloc] initWithFrame:self.bounds configuration: wkWebViewConfig];
#endif // !TARGET_OS_OSX
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    ABI39_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI39_0_0RCTSharedApplication() setStatusBarStyle:UIStatusBarStyleLightContent animated:YES];
    });
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    ABI39_0_0RCTUnsafeExecuteOnMainQueueSync(^{
      [ABI39_0_0RCTSharedApplication() setStatusBarHidden:self->_savedStatusBarHidden animated:YES];
      [ABI39_0_0RCTSharedApplication() setStatusBarStyle:self->_savedStatusBarStyle animated:YES];
    });
#pragma clang diagnostic pop
}
...
#else
- (void)setBackgroundColor:(ABI39_0_0RCTUIColor *)backgroundColor
#endif // !TARGET_OS_OSX
{
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI39_0_0RCTView autoAdjustInsetsForView:self
...
- (void)visitSource
{
    ...
    NSString *html = [ABI39_0_0RCTConvert NSString:_source[@"html"]];
    if (html) {
        NSURL *baseURL = [ABI39_0_0RCTConvert NSURL:_source[@"baseUrl"]];
        if (!baseURL) {
    ...
    // Because of the way ABI39_0_0React works, as pages redirect, we actually end up
    ...
    if (request.URL.host) {
        ...
        NSURL* readAccessUrl = _allowingReadAccessToURL ? [ABI39_0_0RCTConvert NSURL:_allowingReadAccessToURL] : request.URL;
...
-(void)setHideKeyboardAccessoryView:(BOOL)hideKeyboardAccessoryView
{
    ...
    NSString* name = [NSString stringWithFormat:@"%@ABI39_0_0_SwizzleHelperWK", subview.class.superclass];
    ...
    if(newClass == nil)
    {
        ...
        Method method = class_getInstanceMethod([ABI39_0_0_SwizzleHelperWK class], @selector(inputAccessoryView));
...
- (void)postMessage:(NSString *)message
{
  ...
  NSString *source = [NSString
    ...
    ABI39_0_0RCTJSONStringify(eventInitDict, NULL)
  ];
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of ABI39_0_0RNCWebView
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString *))completionHandler{
  ...
  const NSRect ABI39_0_0RCTSingleTextFieldFrame = NSMakeRect(0.0, 0.0, 275.0, 22.0);
  NSTextField *textField = [[NSTextField alloc] initWithFrame:ABI39_0_0RCTSingleTextFieldFrame];
...
{
  ABI39_0_0RCTLogWarn(@"Webview Process Terminated");
  if (_onContentProcessDidTerminate) {
    NSMutableDictionary<NSString *, id> *event = [self baseEvent];
    _onContentProcessDidTerminate(event);
  }
          ...
          thenCall: (void (^)(NSString*)) callback
{
  [self.webView evaluateJavaScript: js completionHandler: ^(id result, NSError *error) {
    ...
    if (error != nil) {
      ABI39_0_0RCTLogWarn(@"%@", [NSString stringWithFormat:@"Error evaluating injectedJavaScript: This is possibly due to an unsupported return type. Try adding true to the end of your injectedJavaScript string. %@", error]);
    }
  }];
}
...
- (void)setMessagingEnabled:(BOOL)messagingEnabled {
  ...
  [
   ...
   injectionTime:WKUserScriptInjectionTimeAtDocumentStart
   /* TODO: For a separate (minor) PR: use logic like this (as ABI39_0_0React-native-wkwebview does) so that messaging can be used in all frames if desired.
      ...
      */
      ...
      for (NSHTTPCookie *cookie in [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookies]) {
        [script appendFormat:@"document.cookie = %@ + '=' + %@",
          ABI39_0_0RCTJSONStringify(cookie.name, NULL),
          ABI39_0_0RCTJSONStringify(cookie.value, NULL)];
        if (cookie.path) {
          [script appendFormat:@" + '; Path=' + %@", ABI39_0_0RCTJSONStringify(cookie.path, NULL)];
        }
...
- (NSURLRequest *)requestForSource:(id)json {
  NSURLRequest *request = [ABI39_0_0RCTConvert NSURLRequest:self.source];
...
@implementation ABI39_0_0RNCWeakScriptMessageDelegate
