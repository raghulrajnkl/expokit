...
@interface ABI39_0_0EXScopedPermissions ()

@property (nonatomic, strong) NSString *scopeKey;
@property (nonatomic, weak) id<ABI39_0_0EXPermissionsScopedModuleDelegate> permissionsService;
@property (nonatomic, weak) id<ABI39_0_0UMUtilitiesInterface> utils;
@property (nonatomic, weak) ABI39_0_0EXConstantsBinding *constantsBinding;

@end

@implementation ABI39_0_0EXScopedPermissions

- (instancetype)initWithScopeKey:(NSString *)scopeKey andConstantsBinding:(ABI39_0_0EXConstantsBinding *)constantsBinding
{
  if (self = [super init]) {
    _scopeKey = scopeKey;
...
- (void)setModuleRegistry:(ABI39_0_0UMModuleRegistry *)moduleRegistry
{
  ...
  _utils = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI39_0_0UMUtilitiesInterface)];
  _permissionsService = [moduleRegistry getSingletonModuleForName:@"Permissions"];
}

# pragma mark - permission requesters / getters

// overriding ABI39_0_0EXPermission to inject scoped permission logic
...
- (NSString *)getScopedPermissionStatus:(NSString *)permissionType {
  if (!_permissionsService) {
    return [[self class] permissionStringForStatus:ABI39_0_0UMPermissionStatusGranted];
  }
...
- (BOOL)hasGrantedScopedPermission:(NSString *)permissionType
{
  ...
  return [_permissionsService getPermission:permissionType forExperience:_scopeKey] == ABI39_0_0UMPermissionStatusGranted;
}
  ...
  if (![globalPermissions[@"status"] isEqualToString:@"granted"]) {
    ...
    UIAlertAction *allowAction = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
      ...
      if (![self.permissionsService savePermission:permission ofType:permissionType forExperience:self.scopeKey]) {
        permission[@"status"] = [[self class] permissionStringForStatus:ABI39_0_0UMPermissionStatusDenied];
...
- (NSDictionary *)getScopedPermissionForType:(NSString *)permissionType withGlobalPermission:(NSDictionary *)globalPermission
{
  ...
  if ([_constantsBinding.appOwnership isEqualToString:@"expo"]
      ...
      && [ABI39_0_0EXPermissions statusForPermission:permission] == ABI39_0_0UMPermissionStatusGranted) {
